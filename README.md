...
| OS | 推奨場所 | 使うスクリプト | 備考 |
|---|---|---|---|
| Windows | WSL / Ubuntu内 | `setup-wsl.sh` | Windows CMD / PowerShellではtmuxが使えないためWSL推奨 |
| macOS | ターミナル | `setup-mac.sh` | Homebrewが必要 |
| Linux / Ubuntu | ターミナル | `setup-wsl.sh` | WSL以外のUbuntuでも使用可能 |
...
そのため、Windowsユーザーは **WSL内でcloneする方法を推奨**します。
...
今回は `codex-company` 自体を対象プロジェクトとして使う前提です。
...
対象プロジェクトへ移動して、`./bin/codex-company` を実行するだけです。
...
PCをシャットダウンしていない、かつ前回のtmuxセッションが残っている場合だけ復帰できます。
...
macOS向けの環境確認スクリプトです。Homebrewを確認し、`git` と `tmux` をインストールし、Codex CLIの存在を確認します。
